...
cwd = Path.cwd()
print(cwd)
data_path = cwd.joinpath('csv', 'alpha_ij.csv')
print(cwd)
...
def update_figure(molecule, valence, Type, View):
    filtered_data = data
    iso_type = 'alphaE'
    # filter the data
    ...
    if View == 'Static':
        omega=0
        filtered_data = filtered_data.query('omega==@omega')
        ...
        #use a symlog axis with linear scale at .1
        #fig.update_yaxes(type="symlog",symlog_lin=0.1)
    else:
        fig = px.line(filtered_data, x='omega', y=iso_type, color='Type', facet_col='ij', facet_row='molecule',
                      color_discrete_sequence=px.colors.qualitative.D3,
                      hover_data=['molecule', 'basis', 'valence', 'omega', 'Type', 'alpha', 'alphaMRA'],
                      render_mode='svg',
                      line_shape='spline',
                      markers=True,
                      width=1000,
                      height=300 * len(molecule),
                      )
        realign_subplot_axes(fig, y_axes=False,x_axes=False)
        fig.update_layout(legend_title_text='Basis Set')
