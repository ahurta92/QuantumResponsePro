...
class DaltonRunner:
    ...
    def __write_energy_input(self, madness_molecule, xc, basis, charge=0):
        ...
        run_dir = (
            self.dalton_dir.joinpath(xc).joinpath(molecule_input).joinpath("energy")
        )
        ...
        if basis.split("-")[-1] == "uc":
            mol_input = mad_to_dal.madmol_to_dalmol(
                madness_molecule_file, "-".join(basis.split("-")[:-1]), charge
            )
        else:
            print(charge)
            mol_input = mad_to_dal.madmol_to_dalmol(
                madness_molecule_file, basis, charge
            )
    ...
    def __write_polar_input(self, madness_molecule, xc, operator, basis, charge=0):
        ...
        run_dir = (
            self.dalton_dir.joinpath(xc).joinpath(molecule_input).joinpath(operator)
        )
        if not os.path.exists(run_dir):
            os.makedirs(run_dir)
        # Here I read the madness mol file from the molecules directory
        ...
        if basis.split("-")[-1] == "uc":
            mol_input = mad_to_dal.madmol_to_dalmol(
                madness_molecule_file, "-".join(basis.split("-")[:-1])
            )
        else:
            mol_input = mad_to_dal.madmol_to_dalmol(
                madness_molecule_file,
                basis,
            )
    ...
    def __run_dalton(self, rdir, dfile, mfile):
        ...
        # dalton [.dal] [.mol]
        if self.use_mpi:
            daltonCommand = dalton + " -N " + str(self.Np) + " " + dfile + " " + mfile
            print(daltonCommand)
        else:
    ...
    @staticmethod
    def __write_excited_input(self, madmol, model, basis, num_states, model_input=None):
        ...
        dalton_inp = "\n".join(dalton_inp)
        if model == "cc":
            model_name = model_input["cc_type"]
        else:
            model_name = model
        run_dir = (
            self.dalton_dir.joinpath(model_name).joinpath(molname).joinpath("excited-state")
        )
        print(run_dir)
        ...
        if basis.split("-")[-1] == "uc":
            mol_input = mad_to_dal.madmol_to_dalmol(
                madmolfile, "-".join(basis.split("-")[:-1])
            )
        else:
            mol_input = mad_to_dal.madmol_to_dalmol(madmolfile, basis)
        dal_run_file = run_dir.joinpath("excited-{}.dal".format(model_name))
        with open(dal_run_file, "w") as file:  # Use file to refer to the file object
            file.write(dalton_inp)
    ...
    def __create_excited_json(output_json, basis):
        ...
        return {
            basis: {
                "ground": e_data,
                "response": s_dict,
                "symmetry": output_json["simulation"]["symmetry"],
            }
        }
    ...
    def get_quad_json(self, mol, xc, operator, basis):
        ...
        run_directory, dal_input, mol_input = self.__write_quadratic_input(
            self, mol, xc, operator, basis
        )
        ...
        try:
            if not beta_json.empty:
                beta_json.rename(
                    columns={"A-freq": "Afreq", "B-freq": "Bfreq", "C-freq": "Cfreq"},
                    inplace=True,
                )

                # now i need to cobine A B C to one column ijk
                beta_json["ijk"] = (
                    beta_json["A"].astype(str)
                    + beta_json["B"].astype(str)
                ...
                f_data = beta_json[
                    beta_json["Beta Value"].apply(lambda x: isinstance(x, float))
                ]
                s_data = beta_json[
                    beta_json["Beta Value"].apply(lambda x: isinstance(x, str))
                ]
    ...
    def get_excited_json(self, mol, model, basis, num_states, model_input=None):
        """
        ...
        run_directory, dal_input, mol_input = self.__write_excited_input(
            self, mol, model, basis, num_states, model_input
        )
        print("dal_input:", dal_input)
        print("mol_input:", mol_input)
        print("run_directory:", run_directory)
        ...
        # you need the _ after model name because dalton writes output files with a dash putting the model name and basis together
        run_name = "excited-" + "{}".format(model_name)+"_" + "-".join([mol, basis]) + ".out"
        json_name = "excited_"+"{}".format(model_name)+"_" + "-".join([mol, basis]) + ".json"
